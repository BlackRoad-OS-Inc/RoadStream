...
> BlackRoad OS fork of Jellyfin
> Jellyfin fork — self-hosted media server for BlackRoad content

## Phase 1 — Q1: Fleet Integration

- Deploy RoadStream to Lucidia (.38) with systemd auto-start
...
- Add NLP intent routing: 'play media' / 'stream'
- Create RoadStream Agent persona with RoundTrip auto-chat
- Integrate with RoadMem for persistent state across restarts
- Add Codex solution entries for common RoadStream operations
- Build blackroad-operator CLI commands: `br RoadStream status|deploy|logs`
...
- Contribute BlackRoad improvements back to upstream Jellyfin
